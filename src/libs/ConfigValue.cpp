...
{
    if( !this->found ) {
        printErrorandExit("could not find config setting, please see http://readybox.co/support\r\n");
    }
    return this;
}
...
float ConfigValue::as_number()
{
    if( this->found == false && this->default_set == true ) {
        ...
        if( endptr <= cp ) {
            printErrorandExit("config setting with value '%s' and checksums[%04X,%04X,%04X] is not a valid number, please see http://readybox.co/support\r\n", this->value.c_str(), this->check_sums[0], this->check_sums[1], this->check_sums[2] );
        }
        return result;
    }
}
...
int ConfigValue::as_int()
{
    if( this->found == false && this->default_set == true ) {
        ...
        if( endptr <= cp ) {
            printErrorandExit("config setting with value '%s' and checksums[%04X,%04X,%04X] is not a valid int, please see http://readybox.co/support\r\n", this->value.c_str(), this->check_sums[0], this->check_sums[1], this->check_sums[2] );
        }
        return result;
    }
}
