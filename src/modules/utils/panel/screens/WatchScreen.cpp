...
void WatchScreen::display_menu_line(uint16_t line)
{
    ...
    switch ( line ) {
        case 0:
        {
            ...
            if(tm.size() > 0) {
                ...
                for (size_t i = 0; i < 2; ++i) {
                    ...
                    THEPANEL->lcd->setCursor(0, i); // col, row
                    //off += THEPANEL->lcd->printf("%s:%03d/%03d ", temp.designator.substr(0, 2).c_str(), t, tt);

                    if(strcmp(temp.designator.substr(0,2).c_str(), "T")  == 0)
                    	off += THEPANEL->lcd->printf("%s %03d/%03d ", "Nozzle Temp:", t, tt);
                    else
                    	off += THEPANEL->lcd->printf("%s %03d/%03d ", "Bed Temp:   ", t, tt);
                }
        ...
        //case 3: THEPANEL->lcd->printf("%19s", this->get_status()); break;
          ...
          case 3: THEPANEL->lcd->printf("Current speed:%3d%%", this->current_speed); break;
          case 4: THEPANEL->lcd->printf("Elapsed Time:%2lu:%02lu", this->elapsed_time / 60, this->elapsed_time % 60, this->sd_pcnt_played); break;
          case 5: THEPANEL->lcd->printf("Percent Done: %3u%% sd", this->sd_pcnt_played); break;

          case 7: THEPANEL->lcd->printf("%18s", this->get_status()); break;
    }
}
